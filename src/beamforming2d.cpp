...
DEFUN_DLD(beamforming2d, args, , "-*- texinfo -*-\n\
## @deftypefn {Function File} {@var{outputImage}= } beamforming2d (@var{hradar}, @var{inputData}, @var{azim_domain}, @var{rho_domain},  @var{algorithm}, @var{force_rebuild_map})\n\
## Beamform the radar data over a desired range\n\
## Input\n\
## @itemize\n\
## @item @var{hradar}: Structure containing stream acquisition parameters\n\
...
## @item @var{hradar}: TxRxCycle: Array containning the active antenna matrix for each acquisition cucle, the array is (2, NumAntennas, NumCycles)\n\
## @item @var{inputData}: stream input (every row is a single stream)\n\
## @item @var{azim_domain}: Reconstruction azimuth domain (cross-range) base\n\
...
## @end deftypefn")
{
    ...
    std::string algorithm = args(4).string_value();
    ...
    if (args.length()==6)
    {
        if (!args(5).is_bool_scalar())
        {
            print_usage();
            return octave_value_list();
        }
        build_remap = args(5).bool_array_value()(0);
    }
    //------------------------------------------------
    ...
    NDArray  azim_base = args(2).array_value();  // Downrange
    NDArray  rho_base  = args(3).array_value();  // Cross range

    int nxx = azim_base.numel();
    int nyy = rho_base.numel();

    octave_idx_type numel_xy = nxx * nyy;
    ...
    ComplexNDArray  beamformed_image(dim_vector({1,numel_xy}),0);
    ...
    int             numel_xy_s     = numel_xy * nstreams;

    if (build_remap)
    {
        // Every array is a 1D array to speed up access. The "internal" order is x,y,z,stream
        indexes_0 = NDArray(dim_vector({1,numel_xy_s}),0);
        indexes_1 = NDArray(dim_vector({1,numel_xy_s}),0);
        coeffs_0  = NDArray(dim_vector({1,numel_xy_s}),0);
        coeffs_1  = NDArray(dim_vector({1,numel_xy_s}),0);
        sin_cos_wp     = ComplexNDArray(dim_vector({1,numel_xy_s}),0);
        ...
        NDArray rtt_s(dim_vector({1,numel_xy_s}));
        double k = 1.0 / C0;
        octave_idx_type index = 0;
        for (int ix = 0; ix < nxx; ix++)
            for (int iy = 0; iy < nyy; iy++)
                {
                    double r = rho_base(iy);
                    double a = azim_base(ix);
                    double xx = r * cos(a);
                    double yy = r * sin(a);
    ...
    // stored
    ComplexNDArray interp_streams(dim_vector({1,numel_xy_s}),std::complex<double>(0.0,0.0));
    ...
    if ((das_selected)||(dmas_selected))
    {
        ...
        for (int ix = 0; ix < nxx; ix++)
            for (int iy = 0; iy < nyy; iy++,  bf_index++)
                {
                    if (das_selected)
                    {
        ...
        radar_struct.assign("beamformed_image", octave_value(beamformed_image.reshape(dim_vector{nyy,nxx})));
