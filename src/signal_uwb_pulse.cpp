...
@deftypefn {} {@var{signal_out}, @var{time} =} signal_uwb_pulse (@var{pulse_shape},@var{tp}, @var{tmax},@var{ts}, @var{code}, @var{prt})\n\
Build a train of UWB pulses (baseband) \n\
...
@end deftypefn")
{
    ...
    if ((pulse_shape == "lt102")||(pulse_shape == "lt103"))
    {
        octave_value_list out(2);
        out(1) = octave_value(time_support);
        out(0) = (octave::feval("interp1", octave_value_list({tpulse, pulse, time_support, "linear",0}))(0));
        return out;
    }
